...
import { BlockheightBasedTransactionConfirmationStrategy, Keypair, LAMPORTS_PER_SOL } from '@solana/web3.js';
...
describe('LocalKeyManager', () => {
    ...
    it('should airdrop 1 SOL to generated key', async () => {        
        ...
        while (!confirmed) {
            ...
            if (status && status[0] && status[0].err === null && status[0].confirmationStatus === 'confirmed') {
                confirmed = true;
            } else {
